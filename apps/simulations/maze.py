...
from apps.simulations.utils import get_maze_meta, read_file_to_list

matrix_path = "the_ville/matrix"
static_matrix_path = finders.find(f"{matrix_path}")
...
class Maze:
    def __init__(self, maze_name):
        ...
        meta_info = get_maze_meta(matrix_path)
        ...
        blocks_folder = f"{static_matrix_path}/special_blocks"
        ...
        maze_folder = f"{static_matrix_path}/maze"
        ...
        for i in range(0, len(collision_maze_raw), meta_info["maze_width"]):
            ...
            self.collision_maze += [[int(v) for v in collision_maze_raw[i : i + tw]]]
    ...
    def print_collision(self):
        ROWS, COLS = len(self.collision_maze), len(self.collision_maze[0])
        for r in range(ROWS):
            for c in range(COLS):
                element = self.collision_maze[r][c]

                if element == COLLISION_BLOCK_ID:
                    print("# ", end="")
                else:
                    print("  ", end="")
            print()
